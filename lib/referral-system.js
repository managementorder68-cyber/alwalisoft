...
exports.processNewReferral = processNewReferral;
exports.distributeReferralCommissions = distributeReferralCommissions;
exports.calculateReferralStats = calculateReferralStats;
 ...
 * معالجة إحالة جديدة
 */
async function processNewReferral(referrerId, referredId, referralCode) {
    try {
        // التحقق من أن المستخدمين موجودان
        const [referrer, referred] = await Promise.all([
            prisma_1.prisma.user.findUnique({ where: { id: referrerId } }),
            prisma_1.prisma.user.findUnique({ where: { id: referredId } })
        ]);
 ...
 */
async function distributeReferralCommissions(userId, amount, description) {
    try {
        // جلب المُحيل المباشر
        ...
        for (let level = 1; level <= 3 && currentReferrerId; level++) {
            ...
            if (!referrer)
                break;
            const commissionAmount = Math.floor(amount * (COMMISSION_RATES[level] || 0));
...
async function calculateReferralStats(userId) {
    try {
        const referralTree = await prisma_1.prisma.referralTree.findUnique({
            where: { userId }
        });
        if (!referralTree) {
            return {
